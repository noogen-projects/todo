...
use todo_tracker_fs::load::Source;
...
fn tasks_from_todo_file() -> anyhow::Result<()> {
    ...
    let plan = todo_tracker_fs::load::project_plan(Source::WholeFile(todo_file_path), &id_generator).unwrap()?;
...
- item 7",
    ...
    let id_generator = IntIdGenerator::new(1);
    let plan =
        todo_tracker_fs::load::project_plan(Source::CodeBlockInFile(manifest_file_path), &id_generator).unwrap()?;
